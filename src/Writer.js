...
const Writer = {
  exportMissionTree: function (series, missions) {
    ...
    for (let index = 0; index < series.length; index++) {
      ...
      //hasCountryShield
      missionTreeText +=
        tabsText + "has_country_shield = " + Writer.boolToYesNo(serie.hasCountryShield) + "\n";
      ...
      for (let index = 0; index < missions.length; index++) {
        ...
        //gerneric
        missionTreeText +=
          tabsText + "generic = " + Writer.boolToYesNo(mission.generic) + "\n";

        //completed_by
        missionTreeText +=
          tabsText + "completed_by = " + Writer.isCorrectValue()
  ...
  handleBlockInput: function (variable, tabsText) {
    ...
    for (let index = 0; index < resSplit.length; index++) {
      ...
      //resSplit[index] = tabsText + element;
      result += tabsText + element + "\n";
    }
    //const regex = /\t/gi;
    //trimmedVar = trimmedVar.replaceAll(regex, tabsText + "\t");
    //result = "{\n" + tabsText + trimmedVar + "\n";
    return result;
  },
